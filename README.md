...
### Windows EXE (easiest)
```
1. Double-click build_exe.bat to build
2. Double-click dist\AIGameTranslator\AIGameTranslator.exe
3. Browser opens automatically — start translating!
```

### Run directly with Python
```bash
pip install -r requirements.txt
python app.py          # Browser opens automatically at http://localhost:5000
```
...
```
ai-translator/
├── app.py           # Web UI server (Flask) — main entry point
...
├── run.bat          # Windows quick launch (double-click)
├── build_exe.bat    # Windows EXE builder (PyInstaller)
├── templates/
...
### Windows EXE（最も簡単）
```
1. build_exe.bat をダブルクリックしてビルド
2. dist\AIGameTranslator\AIGameTranslator.exe をダブルクリック
3. ブラウザが自動で開きます — すぐに翻訳開始！
```

### Pythonで直接起動
```bash
pip install -r requirements.txt
python app.py          # ブラウザが自動で開きます（http://localhost:5000）
```

Windowsの場合: `run.bat` をダブルクリックでもOK。
